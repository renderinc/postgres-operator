	...
	for _, pod := range pods.Items {
		...
		for _, v := range pod.Spec.Volumes {
			if v.Name == "pgdata" {
				...
				printClaim(clientset, v.VolumeSource.PersistentVolumeClaim.ClaimName)
			}
		}
	}
}

func printClaim(clientset *kubernetes.Clientset, claimname string) string {
	...
	fmt.Printf("storage cap string value %s\n", qty.String())
